'''
PrintDebug
9/7/2022
Python I

Instructions
While writing this program, Mr. Pawelski made four mistakes. Find and fix all the mistakes.
When finished, the program should display the following in the console...

Hi there!
My favorite color is magenta.
I love Computer Science!
I also like 3D printing.
I have a cat named Smokey.
'''

print("Hi there!")
print(My favorite color is magenta.)
print"I love Computer Science!"
print("I also like 3D printing.)
print("I have a cat named Smokey."
